...
async def setup_webhook():
    ...
    try:
        ...
        # Wait longer before setting new webhook
        ...
        success = await application.bot.set_webhook(
            ...
            max_connections=2,  # Reduced to prevent overload
            secret_token=None  # No secret token for simplicity
        )
        ...
        if success:
            ...
            if webhook_info.url == webhook_url:
                ...
                try:
                    import requests
                    test_response = requests.get(webhook_url.replace(f"/{TELEGRAM_BOT_TOKEN}", "/uptime"), timeout=10)
                    if test_response.status_code == 200:
                        logger.info("✅ Webhook endpoint is reachable")
                    else:
                        logger.warning(f"⚠️ Webhook endpoint returned: {test_response.status_code}")
                except Exception as test_error:
                    logger.warning(f"⚠️ Webhook endpoint test failed: {test_error}")
...
<b>IP Address:</b> {webhook_info.ip_address or 'Unknown'}
        """.strip()
...
def make_app():
    """Create Tornado application with better timeout settings"""
    settings = {
        "default_handler_class": tornado.web.ErrorHandler,
        "default_handler_args": {"status_code": 404},
        "slow_callback_duration": 10.0,
    }
...
def main():
    ...
    try:
        ...
        # Create server with better timeout settings
        app = make_app()
