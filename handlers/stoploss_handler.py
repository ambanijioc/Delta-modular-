...
from utils.helpers import validate_percentage_input, validate_price_input, calculate_stop_prices
...
class StopLossHandler:
    ...
    async def show_stoploss_selection(self, update: Update, context: ContextTypes.DEFAULT_TYPE, order_id: str):
        """Show stop-loss type selection for a given order"""
        try:
            ...
            if not order_details:
                await query.edit_message_text("❌ Unable to fetch order details. Please try again.")
                return
...
<b>Parent Order:</b> {parent_order.get('symbol', 'Unknown')} ({parent_order.get('side', '').title()})
...
Type your trail amount:
    ...
    async def _ask_limit_price(self, update: Update, context: ContextTypes.DEFAULT_TYPE, 
        ...
        if side == 'sell':  # Long position, selling to exit
...
Type your limit price:
    ...
    async def _get_order_details(self, order_id: str) -> dict:
        ...
        try:
            # This would be implemented based on Delta Exchange API
